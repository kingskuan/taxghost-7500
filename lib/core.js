...
class LiquifyClient {
  ...
  async getTransactions(addresses, options = {}) {
    ...
    const fromBlock  = '0x' + Math.max(0, latest - 10000).toString(16); // ~1 day, fast query

    // Fetch outgoing + incoming ERC-20 transfers in parallel
    ...
    // Get latest block timestamp only (use block offset for approximation — avoids N RPC calls)
    const latestBlock = await this.rpc('eth_getBlockByNumber', ['latest', false]);
    const latestTs    = parseInt(latestBlock.timestamp, 16) * 1000;
    const BLOCK_MS    = 12000; // ~12 seconds per block
    const tsMap       = {};
    for (const log of topLogs) {
      const blockDiff  = latest - parseInt(log.blockNumber, 16);
      tsMap[log.blockNumber] = latestTs - (blockDiff * BLOCK_MS);
    }
